# Quy Ước Commit & Chuẩn Hóa Phát Triển — SWP391 Racehorse Transport

> **Dự án**: `swp391-cross-border-racehorse-transport-system` (Cross-Border Racehorse Transport System)  
...
1. **Commit message (Tiêu đề commit)**:
   ...
   - Ví dụ: `Add health milestone tracker`, `Fix route milestone timestamp parser`.
...
2. **Impact bullets (Các gạch đầu dòng tác động)**:
   ...
   - Mô tả các tác động quan trọng nhất gồm:
     - `behavior`: Thay đổi hành vi tương tác hoặc luồng người dùng (Customer, Specialist, Driver,...).
     - `logic`: Thay đổi thuật toán, xử lý nghiệp vụ, kiểm tra trạng thái hành trình.
     - `i18n`: Thay đổi chuỗi bản dịch đa ngôn ngữ.
     - `removals`: Dọn dẹp, xóa bỏ mã hoặc kiểu dữ liệu thừa.
     - `integrations`: Kết nối API, dịch vụ bản đồ, hoặc cổng thông quan bên ngoài.
