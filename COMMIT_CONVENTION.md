# Quy Ước Commit & Chuẩn Hóa Phát Triển (Commit Convention)

> **Áp dụng cho**: Các dự án trong hệ thống (`meetly`, `swp391-cross-border-racehorse-transport-system`,...)  
> **Cập nhật**: 2026-09-11  
...
1. **Commit message (Tiêu đề commit)**:
   ...
   - Ví dụ: `Add heatmap cell hover tooltip`, `Fix route milestone timestamp parser`.
...
2. **Impact bullets (Các gạch đầu dòng tác động)**:
   ...
   - Mô tả các tác động quan trọng nhất gồm:
     ...
     - `logic`: Thay đổi thuật toán, xử lý nghiệp vụ, kiểm tra trạng thái.
     - `i18n`: Thay đổi chuỗi bản dịch đa ngôn ngữ.
     - `removals`: Dọn dẹp, xóa bỏ mã hoặc kiểu dữ liệu thừa.
     - `integrations`: Kết nối API, SignalR hub, dịch vụ bên ngoài.
