...
## Cross-Border Racehorse Transport System (`swp391-pegaxus`)

> **Mã đề tài / Phụ trách**: `2 — HoangNT20`  
> **Dự án**: Cross-Border Racehorse Transport System (`swp391-pegaxus`)  
> **Nguồn dữ liệu**: Excel `UC List Brainstorm (1).xlsx`  
> **Định dạng**: Đánh số Use Case duy nhất trên toàn bộ hệ thống (UC-01 đến UC-77)  
> **Cập nhật**: 14/09/2026 (Bản chuẩn hóa 77 Use Cases theo 6 Flows nghiệp vụ)  
...
|---|---|---|---|
| **Customer** | Khách hàng (Chủ ngựa / CLB đua) | `ROLE_CUSTOMER` | Đăng ký/đăng nhập, tạo yêu cầu vận chuyển, tải hồ sơ y tế/hộ chiếu ngựa, theo dõi hành trình realtime, nhận thông báo thông quan, ký biên bản bàn giao và gửi khiếu nại/claim. |
| **Logistics Manager** | Quản lý Điều hành Logistics | `ROLE_LOGISTICS_MANAGER` | Tiếp nhận & phê duyệt đơn hàng; lập kế hoạch tổng thể; phân công Specialist, Coordinator, Driver, Escort; phê duyệt thay đổi lộ trình & chi phí khẩn cấp; xem báo cáo doanh thu, chi phí, OTD, sự cố. |
| **Transport Specialist** | Chuyên viên Thủ tục & Kiểm dịch | `ROLE_TRANSPORT_SPECIALIST` | Quản lý quy định pháp lý/kiểm dịch từng quốc gia; khởi tạo & quản lý hồ sơ số hóa của ngựa (Hộ chiếu ngựa, Giấy chứng nhận sức khỏe/tiêm phòng); nộp và theo dõi duyệt từ cơ quan chức năng; xác nhận đủ điều kiện khởi hành. |
| **Fleet & Route Coordinator** | Điều phối viên Đội xe & Lộ trình | `ROLE_FLEET_COORDINATOR` | Quản lý đội xe chuyên dụng và khoang/thùng chứa; thiết kế lộ trình tối ưu và các điểm dừng nghỉ/kiểm dịch; theo dõi tiến độ chặng; tái lập lộ trình tránh sự cố; đề xuất chi phí phát sinh. |
| **Vehicle Driver** | Tài xế Vận chuyển | `ROLE_VEHICLE_DRIVER` | Vận hành phương tiện chở ngựa an toàn; xem lịch trình & danh sách điểm dừng nghỉ được phân công; cập nhật trạng thái mốc chặng di chuyển (xuất phát, dừng nghỉ, đến điểm); báo cáo sự cố xe/giao thông. *(Chỉ phụ trách lái xe & vận hành phương tiện)*. |
| **Escort** | Nhân viên Đi kèm & Chăm sóc Ngựa | `ROLE_ESCORT` | Trực tiếp đi cùng ngựa trên xe/máy bay; kiểm tra và ghi nhật ký sức khỏe/tâm lý ngựa; chụp ảnh và ghi chú tình trạng ngựa tại các mốc; báo cáo sự cố sức khỏe ngựa khẩn cấp; tham gia bàn giao tình trạng sức khỏe ngựa tại điểm đến. |
| **System** | Hệ thống Tự Động | `SYSTEM` | Tính toán cước phí tự động, gửi thông báo OTP/Email, đồng bộ cảnh báo GPS, tổng hợp dòng chi phí booking, tự động chuyển trạng thái hóa đơn khi thanh toán. |
...
|:---:|---|---|---|
| **UC-01** | Đăng ký tài khoản khách hàng | Cho phép người dùng bên ngoài (Chủ ngựa, Câu lạc bộ) cung cấp thông tin (Họ tên, Email, Mật khẩu, Tên CLB) để tạo tài khoản mới. Hệ thống kiểm tra trùng lặp, gửi mã/link xác thực qua Email và khởi tạo hồ sơ Customer trên hệ thống. (Lưu ý: Nhân sự nội bộ không tự đăng ký mà sẽ được cấp tài khoản). | Customer |
| **UC-02** | Đăng nhập và phân quyền | Cho phép người dùng nhập thông tin xác thực (Email và Mật khẩu). Hệ thống kiểm tra dữ liệu, cấp phiên làm việc (token) và điều hướng giao diện (phân quyền) tương ứng với role của người dùng (Customer, Manager, Specialist, Coordinator, Driver, Escort...). | All Actors (Customer, Logistics Manager, Transport Specialist, Fleet & Route Coordinator, Vehicle Driver, Escort) |
| **UC-03** | Quên / Khôi phục mật khẩu | Cho phép người dùng yêu cầu khôi phục quyền truy cập khi quên mật khẩu. Người dùng nhập Email, hệ thống gửi mã xác thực hoặc link khôi phục qua Email. Sau khi xác thực thành công, cho phép người dùng đặt lại mật khẩu mới. | All Actors |
| **UC-04** | Thay đổi mật khẩu | Cho phép người dùng đang trong phiên đăng nhập (đã xác thực) đổi mật khẩu tài khoản để tăng tính bảo mật. Người dùng cần nhập đúng mật khẩu cũ và xác nhận mật khẩu mới. | All Actors |
| **UC-05** | Cập nhật hồ sơ cá nhân (Profile) | Cho phép người dùng xem và chỉnh sửa các thông tin cá nhân cơ bản (Ảnh đại diện, Địa chỉ liên hệ, Thông tin liên lạc). Hệ thống lưu vết chỉnh sửa và cập nhật dữ liệu mới nhất. | All Actors |
| **UC-06** | Đăng xuất hệ thống | Cho phép người dùng chủ động chấm dứt phiên làm việc hiện tại. Hệ thống tiến hành xóa token, hủy quyền truy cập và điều hướng người dùng về lại màn hình Đăng nhập. | All Actors |
...
- `UserAccount`: `id`, `email`, `passwordHash`, `fullName`, `phone`, `role` (CUSTOMER, LOGISTICS_MANAGER, TRANSPORT_SPECIALIST, FLEET_COORDINATOR, DRIVER, ESCORT), `isActive`, `createdAt`
- `CustomerProfile`: `id`, `userId`, `clubName`, `businessRegistrationNumber`, `billingAddress`
- `TransportBooking`: `id`, `bookingCode`, `customerId`, `originLocation`, `destinationLocation`, `requestedPickupTime`, `status` (PENDING_APPROVAL, APPROVED, REJECTED, CANCELLED), `approvedBy`, `approvedAt`, `rejectionReason`, `createdAt`
- `BookingHorseItem`: `id`, `bookingId`, `horseName`, `microchipNumber`, `passportNumber`, `specialRequirements`, `dietNotes`, `stallPreference`
- `BookingStatusHistory`: `id`, `bookingId`, `fromStatus`, `toStatus`, `performedBy`, `performedAt`, `reason`
...
- `LegalRegulation`: `id`, `countryCode`, `regulationType` (CUSTOMS, QUARANTINE, WELFARE_EC12005), `title`, `description`, `effectiveDate`, `isActive`
- `DossierTemplate`: `id`, `templateName`, `transportScope` (DOMESTIC, CROSS_BORDER), `description`, `isActive`
- `DossierTemplateItem`: `id`, `dossierTemplateId`, `documentType`, `isRequired`, `notes`
- `TripDossier`: `id`, `tripPlanId`, `horseId`, `templateId`, `status` (PENDING_DOCS, IN_REVIEW, SUBMITTED_AUTHORITY, APPROVED, REJECTED)
- `DossierDocument`: `id`, `tripDossierId`, `documentType` (HORSE_PASSPORT, HEALTH_CERTIFICATE, CITES_PERMIT, CUSTOMS_DECLARATION), `documentNumber`, `issuedDate`, `expiryDate`, `issuingAuthority`, `fileUrl`, `version`, `verificationStatus` (PENDING, APPROVED, REVISION_REQUESTED), `reviewerId`, `reviewerNotes`
- `AuthoritySubmissionLog`: `id`, `tripDossierId`, `authorityName`, `submissionDate`, `referenceCode`, `submissionProofUrl`, `reviewStatus` (SUBMITTED, UNDER_REVIEW, APPROVED, REJECTED), `feedbackNotes`
- `BorderInspectionLog`: `id`, `tripPlanId`, `checkpointName`, `inspectionType` (VET_QUARANTINE, CUSTOMS_CLEARANCE), `inspectionResult` (PASSED, FAILED, RECHECK_REQUIRED), `inspectorNotes`, `clearedAt`, `loggedBy`
...
- `TripPlan`: `id`, `bookingId`, `planCode`, `overallStatus` (DRAFT, PROPOSED, APPROVED, IN_PROGRESS, COMPLETED, CANCELLED), `estimatedStartTime`, `estimatedEndTime`, `managerId`, `createdAt`
- `Route`: `id`, `tripPlanId`, `routeName`, `isContingency`, `parentRouteId`, `totalDistanceKm`, `estimatedDurationHours`, `status` (DRAFT, PENDING_APPROVAL, APPROVED, ACTIVE, SUPERSEDED)
- `RouteCheckpoint`: `id`, `routeId`, `sequenceOrder`, `checkpointType` (ORIGIN, REST_STOP, BORDER_CONTROL, FERRY_TERMINAL, DESTINATION), `locationName`, `latitude`, `longitude`, `plannedArrivalTime`, `plannedDepartureTime`, `mandatoryRestMinutes`
