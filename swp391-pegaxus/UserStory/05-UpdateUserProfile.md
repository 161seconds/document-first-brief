...
- **Story**: Là một Người dùng hệ thống, tôi muốn xem và cập nhật các thông tin cá nhân cơ bản (Ảnh đại diện, Địa chỉ liên lạc, Tên CLB/Trang trại) để đảm bảo dữ liệu luôn chính xác phục vụ việc liên lạc và lập chứng từ vận chuyển.
...
1. Hệ thống tải và hiển thị thông tin hồ sơ hiện tại của người dùng:
   ...
   - Thông tin chi tiết từ bảng `CustomerProfile` (nếu là Customer) hoặc hồ sơ nhân sự: Ảnh đại diện (`avatarUrl`), Địa chỉ liên lạc (`contactAddress`), Tên Câu lạc bộ/Trang trại (`clubName`), Mã số thuế (`taxCode`).
2. Người dùng thực hiện chỉnh sửa các trường được phép:
   ...
   - Chỉnh sửa Họ tên, Địa chỉ liên lạc, Tên CLB hoặc Mã số thuế.
...
4. Hệ thống kiểm tra định dạng dữ liệu đầu vào:
   ...
   - Họ tên và Địa chỉ liên lạc không để trống và có độ dài hợp lệ.
