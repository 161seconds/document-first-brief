...
```mermaid
flowchart TD
    SOS[Tài xế nhấn SOS trên Ứng dụng Di động] --> Level{Phân loại Mức độ SOS}
    Level -->|Cấp 1: Thú y Nguy kịch| VetSOS[Kích hoạt Thú y Lưu động Khẩn cấp trong bán kính 50km]
    Level -->|Cấp 2: Hỏng hóc Kỹ thuật / Xe| TruckSOS[Điều xe cứu hộ & Xe tải chuyên dụng thay thế]
    Level -->|Cấp 3: Tắc nghẽn Cao tốc > 4h| RouteSOS[Tái lập lộ trình khẩn sang Trạm dừng nghỉ gần nhất]
    Level -->|Cấp 4: Ngựa Kích động Phá chuồng| SedateSOS[Chỉ dẫn dùng thuốc an thần khẩn cấp qua điện thoại]
    
    VetSOS --> NotifyAll[Hệ thống tự động gửi SMS & Notification cho LM, TS, Customer]
    TruckSOS --> NotifyAll
    RouteSOS --> NotifyAll
    SedateSOS --> NotifyAll
```
